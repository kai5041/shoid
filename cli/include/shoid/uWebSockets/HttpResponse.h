...
private:
  ...
  bool internalEnd(std::string_view data, uintmax_t totalSize, bool optional,
    ...
    if (httpResponseData->state & HttpResponseData<SSL>::HTTP_WRITE_CALLED) {
      ...
      /* Remove onAborted, onWritable function and mark done if we reach the
       * end, or if we were given no data (faked size like in HEAD response) */
      /* I need to figure out if this line should rather be simply
       * httpResponseData->offset == data.length() */
      /* No that can't be right, tryEnd with fake length should not complete the
       * response even if the smaller chunk wrote in one go */
      /* Possibly need  to separate endWithoutBody and tryEnd with fake length
       * into two separate calls with a boolean that explicitly marks isHeadOnly
       */
